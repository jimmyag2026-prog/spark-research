...
| H-8 | **V141** | CI runs only `unit` + binary smoke; `concurrency`, `timeout`, `integration`, `e2e`, `sdk` suites never run in CI, and `package.json` has no `test:concurrency`/`test:timeout` scripts | `.github/workflows/ci.yml`: only `bun test tests/unit` and `bash scripts/smoke-binary.sh`. `package.json:28-36`: scripts exist for `test`, `test:integration`, `test:lab`, `test:py`, `test:e2e`, `test:sdk` — but no `test:concurrency` or `test:timeout`, and none of `integration`/`e2e`/`sdk`/concurrency/timeout are wired into the workflow. | Add the two missing npm scripts (pointing at whatever directories the existing `tests/concurrency` and `tests/timeout` folders use today — verify exact paths before writing). Add CI jobs for `integration`, `e2e` (headless), `sdk` (py), and `concurrency`+`timeout`. Budget for `e2e`/`sdk` needing the Python venv step already present for `test:lab`. | CI green on the PR that adds it; deliberately break one covered suite locally first to confirm the new CI job actually catches it (repo convention: "smoke must not go through a pipe that swallows the exit code" — same discipline applies to newly wired suites). |
...
main @ v0.8.0
   ...
   │        H-8 CI suite coverage + npm scripts
   │
