...
| V27 | **单二进制里 `import.meta.dir` 全线失效（阻塞 npx 分发）** | v0.4 W1 收口实测：`bun build --compile` 的产物里 `import.meta.dir` 指向虚拟的 `/$bunfs/root/`，**任何靠它拼路径的代码在二进制里都读不到文件**。现状 **23 处、17 个文件**（`kernels/manager.ts` 找 python 脚本、`lab/wet_backend.ts` 找 `opentrons_backend.py`、`agents/*` 找 prompt `.txt`、`simulation/*` 找适配器脚本、`skills/frontmatter.ts`、`scaffold/templates.ts`、`server/app.ts` 找前端产物…）。已修两处（`index.ts` 与 `version.ts` 改静态 import，并加了版本号一致性断言）；**其余未修，后果是二进制里干湿实验 / prompt 加载 / 脚手架 / 前端托管大概率全坏**。实测证据：`./dist/spark-research doctor` 报 `/$bunfs/root/opentrons_backend.py: No such file`。<br>**这是 v0.4「一条 npx 命令」交付的硬阻塞**，发 v0.4.0 前必须解决，或明确降级承诺（只发 npm 包、不发单二进制）。修法方向：Bun 的 embedded files、资产改静态 import、或运行期解包到可写目录。 |
