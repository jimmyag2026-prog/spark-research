...
## 外部验收结果（2026-09-09，主会话执行）

派一个**对本仓库一无所知、且被禁止读源码**的 agent，只给 MCP 启动方式与 llms.txt，让它完成「建项目 → 检索入库 → 提 idea → novelty check」。

**结果：8/10，全链路真实跑通。** 它建了 `protein-structure-confidence` 项目、入库 10 篇（AlphaFold 系列 / QMEAN 系 / 通用 ML 校准，命中该主题的核心文献）、生成 10 张精读卡、共探出一张 Idea 卡（保形预测校准 pLDDT/PAE），novelty check 4 条 claim 中 3 条 novel、**1 条被确定性校验层从模型给的 novel 强制改判为 existing**（检索到真实论文 CalPro，相似度 0.78 ≥ 0.75 阈值）。AD-8 在一次完全独立的使用中被验证生效。

它没被要求做精读，却自己判断出精读卡是提 idea 的基础并主动调用——说明工具描述里的「典型链路」写对了。

### 暴露的三个真问题（已在 v0.2.1 修复）

1. **长任务句柄的跨连接语义与文档承诺不符（最严重，属诚信问题）**。`task_status` / `exp_run` 的描述写着「任务仍在后台跑」，但 `TaskRegistry` 是纯内存 Map：连接一断句柄即失效。对 `exp_run` 磁盘上还有状态可 resume，对 `lit_read_cards` 这类**没有磁盘 checkpoint 的长任务则是真丢**。验收 agent 原话：「不写清楚很容易让人以为『我关了终端任务还在跑，明天再来查』，实际不是这样。」
   → 修：两处描述改为讲清「句柄只在本连接有效」，并**分别**给出干实验（exp_list + resume）与文献类任务（需重跑）的处置方式；404 错误信息本身也带上原因与下一步。
2. **`lit_read_cards(all=true)` 不跳过已读**，超时重跑会把已生成卡片的论文重烧一遍模型调用，代价随库线性增长。
   → 修：批量精读默认跳过已读，新增 `redoRead` 参数强制重生成；全部已读时明确报错而非静默空跑。
3. **`ideaId` 与 `stored.recordId` 命名在两个工具间不一致**，验收 agent 要往下挖才找到该传什么。
   → 修：返回体加顶层 `ideaId` 别名（保留 `stored.recordId` 向后兼容），描述里点名。

三条各配回归测试（`tests/unit/mcp_friction_fixes.test.ts`，9 例），外加一条护栏断言修描述没碰坏 AD-9 的五个扣留工具。

### 验收 agent 明确肯定的部分

工具描述五段式（【何时调】【参数示例】【何时不该用】【典型链路】【读法】）——「几乎没有一次因为不知道参数怎么填而卡住」；检索源失败分档 ok/skipped/failed 且 S2 的 429 在 capabilities 里**提前警告**而非事后报错；novelty 校验层的 `violations` 字段直接说明「评级从 novel 被改判为 existing，因为相似度 0.78 超过 0.75 阈值」；扣留工具清单把「人该做什么」讲清楚。它的评价是这几点「在其他 MCP 工具里很少见到」。

### 一个意外的架构验证

验收中途 repo 目录被外部进程重命名，它的调用路径失效——但 `~/.spark-research/` 数据目录与 repo 代码目录分离，项目数据完全没受影响，指向新路径后一切照常。这条分离不是为此设计的，但在真实故障里救了场。
