...
**日期** 2026-09-16 · **分支** `fix/ux-U38-U40` · 触发：owner 首次以普通用户身份用 v0.9.0，网页端 chat 问「帮我下载 mRNA × AI 的综述」，0 篇 0 PDF。现场记录 `docs/UX_TEST_v0.9.0.md`，条目 `USAGE_LOG.md` U38–U43。
...
| **U40** | `literature.ts` 新增 `assertSearchPayload()`，接在 EuropePMC 的 `search` 出口。判据只查**计数或结果容器在不在**，不查是不是 0 条——0 条是合法结果，语法错不是 | `ux_window` U40 ×3 | 判据恒真 → **1 红** |

## 如实交代

- `assertSearchPayload` **目前只接在 EuropePMC 一处**。其余源（pubmed/openalex/crossref/arxiv/semanticscholar）的响应形状各不相同，没有现场证据就照着抄一遍，等于凭想象加判据；函数已导出，遇到第二例再逐个接，届时把 `COUNTERS` 收敛成按连接器的显式表。
