...
| F | 搜索只按面板名匹配，不看面板内设置项 | e2e ㉞ 红（`llmtimeoutms` 搜不到 general） |

F 的输出：

```
✘ 1 ㉞ 设置面搜索框：既过滤面板名，也过滤面板内设置项；没建索引的面板不隐藏 (15.6s)
  Error: expect(locator).toHaveCount(expected) failed
  Expected: 1
  Received: 0
  > 1641 |   await expect(general).toHaveCount(1);
  1 failed
```

复原后全绿：`50 passed (26.1s)`。

### 搜索索引的真实覆盖范围（㉞ 钉的就是这件事）

索引由面板自己在拿到 API 数据后登记（`SettingsPanelProps.register`，文本来自 API 响应，
不是前端另编的），所以**只覆盖本次打开过的面板**；`general` 是默认面板、壳一开就挂，
那 32 个配置键永远可搜——U6 的主诉正是这 32 个键。没建索引的面板在有搜索词时**不隐藏**，
降一档显示并在导航项上标 `?`：搜不到不等于里面没有，藏掉就是做一个兑现不了的承诺。
㉞ 的 ③④ 两步把这个行为钉死（unindexed 存在 → 打开该面板后它就参与真匹配）。

彻底的解法要后端给一条总索引路由，见「给 γ 的契约请求」。

### 步骤 5 · γ 实装落地后回头对账（契约没改名，但 `extra` 变了）

γ 后来推到 `cbd9eb0`（十个面板填实现 + V130 + `includeArchived` + `auth --connector`）。
回头核对：

- `types.ts` 与骨架 commit `d2fa8d4` **逐字节相同**——契约冻结守住了，一个字段都没改名。
- `GET /api/projects` 实装是 `queryBool("includeArchived") || queryBool("all")`，与我这边
  两个参数都发的写法正好对上，中间没有一段时间下拉框会突然多出验收产物。

**但 `extra` 里的键变了**（契约允许「只增键」，可它们不是新增，是与骨架 fixture 里的名字
不同）。这是唯一会让前端**静默渲染成空**的一类漂移，逐条对过：

| 面板 | 骨架 fixture | 实装 | 不改的后果 |
|---|---|---|---|
| extensions | `category: "mcp"` / `"skill"` | `category: "extension"` / `"skill"`，key 带 `ext:` / `skill:` 前缀 | **连接器面板整个空一片**，且没有任何报错 |
| extensions | `tools` | `mcpTools` | 发现到的工具名不显示 |
| extensions 写路由 | key == 名字 | 路径参数要**裸名字**，key 是 `ext:<name>` | 验证/卸载打到 `ext:foo`，404 或空操作 |
| models | `extra.subAgent` | `extra.subAgentType` | 子代理覆盖写不进去（会退回按键名前缀，恰好还能работать，但不该靠巧合） |

改法一律是**两代名字都认**（实装名优先，骨架名兜底），不是把旧的换掉——契约冻结的承诺是
「只增字段不改名」，前端对这条承诺的兑现方式就是不假设只有一种名字。`ext:` 前缀在调写
路由前剥掉，剥的是 `key` 不是 `label`（label 是给人看的，随时可能变成带版本号的展示名）。

e2e 假件同步改成**实装的形状**（不是骨架的），并在写路由上加了一道：名字里带 `:` 就返回
错误——前端要是忘了剥前缀，㉘ 会红而不是静默通过。

这一段是本 lane 最接近「差点交出一个静默空面板」的地方。**教训**：契约冻结保的是字段名，
保不了 `Record<string, unknown>` 里的约定；`extra` 这类逃生舱口必须在对方实装落地后回头
再对一次账，光读冻结点的 fixture 不够。
