...
- **自己踩的一个坑，记下来**：`globToRegExp` 第一版用两个不可打印字符当占位符（「先把 `*` 换成占位符，转义完再换回 `.*`」），结果**真实的控制字符被写进了 `slug.ts`**，git 直接把这个 .ts 当二进制处理（`Bin 1097 -> 2079 bytes`，diff 看不见内容）。测试全绿，所以只有在收尾核对 `git diff --stat` 时才暴露。已改成一次扫描按字符分支，文件恢复纯文本，行为不变。教训：占位符法在源码里用真控制字符是有代价的，门禁不会替你发现它。
