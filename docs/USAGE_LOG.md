...
<a id="u57"></a>
## U57 · skill 摘要被 600 字符上限切掉后半段，汇总模型看不到精读卡与综述 artifact

**现场**：2026-09-16 00:02，用户问「近 6 个月中国团队在 auto-research 方向的进展」（session `web_1789482658926`）。回复说「执行日志中未包含任何论文级明细…也未记录 t2 阅读卡片与 t3 最终综述的实际内容或任何工件 ID」。

**证据**：工作区 `t2_litreview.json`：mode=review、digest 1500 字符、卡 8 张、综述 artifact `545eef26-67d7-44d8-a4d3-424113f51e30`。summarize 收到的 skill 块实测 **629 字符**，不含「命中样例」「精读卡」「artifact」——U48 修 summarize 时给非连接器输出设了 `slice(0, 600)`，而文献流程的摘要把最要紧的三行放在末尾。规划器另排了 `t3_synthesis`（analysis）去「基于 t2 的阅读卡片综合」，analysis 任务读不到卡，只能写空话。

**问题**：修 U48 时把「连接器 JSON 要摘要」推广成「所有输出截 600」，没有区分**本身就是摘要**的 skill 产出（AD-17 的反面：接线了但读得不全）。

**修改方向（已做）**：skill 输出上限 2000（摘要本身 ≤1500）；plan 提示词明说 literature-review 已产出综述 artifact，不要再排 analysis 综合任务。门禁：6 查询 × 8 命中的 digest（>700 字符）到 summarize 时含「命中样例」；阴性对照 cap 退回 600 → 红。


<a id="u58"></a>
## U58 · 中文检索词命中多、查准率接近零

**现场**：2026-09-16 用户问「中文文献也可以搜索和下载吗」。实测 `lit search "重复性劳损 预防 办公人群" --sources aminer,openalex,europepmc --limit 6`。

**证据**：75 条原始结果 → 74 条；前 5 条依次是丙肝防治指南、IMF 中国国别报告、ChiMed-GPT、白塞病中西医综述、新质生产力与体育科技——**没有一条与重复性劳损有关**。AMiner 自报「原查询 0 命中…退回单词命中（查准率低）」；OpenAlex/Europe PMC 对中文 token 做的是松散匹配。CNKI/万方是 `placeholder`（U46），一次请求都不发。

**问题**：平台对中文查询没有任何处理层——既不英译、不抽主题词，也不按语言/期刊过滤；三个源里只有 OpenAlex 真正索引中文期刊（有 DOI、部分有 OA PDF），但没有相关性保障。用户看到的是「能搜」，拿到的是噪声。

**修改方向**：并入 v0.10 γ-2（V161）并扩大口径为「中文查询处理」：① 中文查询先做主题词抽取 + 英译（一次便宜 LLM 调用或词典），中英双查后合并；② OpenAlex 加 `language:zh` / 中文期刊 ISSN 过滤开关；③ 批量预筛（S9）对中文结果同样生效；④ 文档明写 CNKI/万方不可用（U46 已写）。**下载**：OpenAlex 索引到的中文 OA 期刊（如上例 `实用医学研究`）有 PDF 直链可下；CNKI 内容不可能。
