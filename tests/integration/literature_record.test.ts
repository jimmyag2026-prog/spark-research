...
//   平时（含 CI）：默认 replay 模式 → **照常执行**，回放 tests/fixtures/literature/*.json，零网络
//                  （另有一套独立的回放用例在 tests/unit/literature_e2e.test.ts，覆盖面不同，不是本文件的替代）
//
...
describe("文献链路 · 跨源检索 / DOI 取单篇 / PDF 下载 / AMiner 降级（FIXTURE_MODE 决定回放还是打网络）", () => {
  test(
    "跨源检索四个免 key 源",
    async () => {
