...
const ALLOWED_ORPHANS: Record<string, string> = {
  ...
  // backend/src/agents/contract.ts 曾在此登记「等接线：W3-a（replan 循环）接上后必须
  // 删除本条」——W3-a 的 backend/src/agents/orchestrator.ts 现在
  // `import { createLiteratureReviewContract, RecordStoreEvidenceQuery, NoProgressGuard,
  // evaluateRound, describeStop, ... } from "./contract"`，并在新增的
  // `OrchestratorAgent.runResearchLoop()`（消费 backend/src/agents/replan.ts 的
  // `runReplanLoop()`）里真实构造契约、对证据图跑 `evaluate()`、用 `evaluateRound()`
  // 判三条并行停机条件之二——contract.ts 有了真实生产调用方，按对称检查删除本条。
