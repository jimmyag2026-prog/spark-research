...
- **一轮 chat 没有变快。** R6 基线：一句话问题 P50 27–101s、每轮 3–5 次模型调用；机制已量清楚（`docs/devlog/R6-baseline.md` §机制解释：109.6s 里 78% 是模型在生成 8590 个输出 token；chat 对任何问题都走完整 plan→execute→summarize，各阶段无输出上限）。A8 侧复测 P90 三降一升（`docs/devlog/A8-baseline.md`），是上游生成速度波动，不是本版的功劳——DONE 第 2 条走的是「机制解释」分支。压它是 v0.10 的第一件事（直答路径 + 各阶段 `maxTokens`）。
- 单轮 chat 超过 255s 仍会被 server 掐断（V156，须裁定方向）。
- T3 脑机接口课题召回仍 0/8（AMiner 中文检索塌陷，V161）。
- 「人为断网 30s 内报错」两轮验收都没有真断网的权限，用的是不等价替代探针（无 key 模型 / 黑洞代理）；替代探针下 alpha.3 挂到 255s，v0.9.0 已修为 14s 内返回（U29）。
- A8 报的「凭据删除按钮空操作」复核**不成立**（Playwright 真实路径可删；验收探针等的是原生 confirm）。
- 登记不做：V155–V170。

### 亮点

- **网页端设置面（12 面板）**：所有非密配置可在浏览器里改、**即时生效**（U6、U15）；凭据经 loopback-only 写路由录入，只写不读，写后任何响应/日志/数据目录零泄露，远端 Origin 与非回环传输层一律 403 且不受 originAllowlist 影响（AD-18，A8 用 56 条响应体全量断言 + 1528 文件逐字节扫描验过）。
- **模型控制面**：单价表即模型清单，未登记的模型名写入时就拒（U5）；显式指名而 provider 无 key → 直接报错并点名变量（V154）；`chat --model/--budget-usd/--project/--help`，`--help` 零调用（U9）；`model` 覆盖真的生效（U10）。
- **失败可归因**：看门狗（静默/总时长）+ 分级重试；每条失败带 `errorKind`（auth / rate_limit / timeout / network / upstream / unsupported / budget）；预算闸拒绝是结构化 `failure` 字段 + 台账行，不再是 200 + approved（U1 U12 U21 U29）。
- **门禁 AD-17「声明即须有读者」**：TypeScript 编译器 API 找出「声明了、传了、静默丢弃」的字段——U10/V137/V40 那一族第一次被机器抓到。
- 阶段化进度事件（U4）、`doctor` 探运行实例（U2）、集成套件缺前置即红（U7）、`/api/usage` 全局汇总（V130）、`auth --connector`、R6 基线测量脚本。

### 数字

- 单元测试 2591 → 2652；e2e 25 → 50；新增门禁文件 8 个；三轮阴性对照共 9 条实跑变红。
- 两轮零上下文验收：R6（T1–T5，$0.044）+ A8（十行验收表，$0.004）；USAGE_LOG U1–U37，其中 U1 U2(半) U4 U5 U6 U7 U8 U9 U10 U11 U12 U15 U21 U22 U23 U28 U29 U30 已关。
- 召回与 v0.8 R5 并排：T1 5/8（持平）· T2 3/8（+1 待核实）· T3 0/8（持平）· T4 4/8（持平）。

### A8 修复窗口（v0.9.0 相对 alpha.3 的改动）

- **凭据写路径对 `Origin` 单独卡回环**（U28，A8 Blocker）：被加进 originAllowlist 的远端页面此前能在本机改写/删除凭据。
- **规划调用失败即止**（U29）：上游连不上时不再退到默认计划去跑连接器任务，`failure.kind = "llm"`，14s 内返回。
- **审批令牌接受 ≥8 位唯一前缀**（U30）：UI/CLI 印的是短 id，令牌按全 UUID 绑定，此前照文档操作必撞「令牌无效」。
...
一天真实使用撞出十条问题（`docs/USAGE_LOG.md` U1–U10），复盘发现同一个病反复出现在三个部位：
可写入的配置项没有读者（V40）、类型上的行为开关字段没有读者（V137：整个代码库零重试）、
公开函数的对象参数属性没有读者（U10：`chat()` 的 `model` 传什么都静默无效）。
三次都是在使用中撞出来的，不是被门禁抓到的。先修单个实例等于承认还会有第四第五个——
所以先建门禁，让它把这一类的全体人口找出来。

### 两条新门禁

- `tests/unit/gate_i_switch_readers.test.ts`：`llm/types.ts` 全部 boolean/number 字段自动入网，
  在 `backend/src` 找读取点。剥注释；同名转发（`retryable: x.retryable`）不算读，异名映射
  （`max_tokens: o.maxTokens`）算读——第一版把所有转发都排除，误杀了 `maxTokens`，已收紧。
- `tests/unit/gate_i_param_readers.test.ts`：TypeScript 编译器 API 解析函数体，不靠 grep。
  同类内 `this.m(param)` 转发一跳可追、按条件位置分桶；**本函数自己读了 ≥3 个属性却只在某个
  分支的转发里读到某属性 → 「主路径必须读」单独断言**。这正是 U10 的精确形状。
- 两条都有 allowlist 两向检查：未登记的孤儿红、已有读者的陈旧登记也红。

### 首跑抓到的（main@644cccd，全部真跑）

| 发现 | 去向 |
|---|---|
| `maxRetries`（V137 本尊） | 闸门 H（#109）合入后已转绿——门禁的历史对照被活体验证 |
| `ProviderCapabilities` 的 `toolCalling` / `streaming` / `usageReported`：构造了无人消费 | **V143**，lane α / 收口接线 |
| `WetLabLoop.execute` 的 `note` 声明了从未读 | **V144**，lane δ |
| `chat()` 的 `model` 只在 coexplore 分支转发时被读 | **V145**（= U10），lane β-1 |
| 门禁能力边界：具名 `type` 参数不在扫描面；同名解构会被算成读者（`retryable` 因此漏报） | **V146** |

### 如实交代

- 四条新发现在 allowlist 里带原因登记，**alpha.1 的绿是登记出来的绿**；对应 lane 接线后陈旧检查会逼着删登记。
- 形状 ③ 是文本级启发式，`anthropic.ts` 里 `const { kind, retryable } = classifyHttpError(...)` 被算成读者，
  所以 `retryable` 在 v0.8.0 上不进红名单；V137 的历史对照由 `maxRetries` 承担。
- 本版同时收入 V142：**CI 自 v0.8.0 起就是红的**（`v118_openmm_probe.test.ts` 假设 `.venv/bin/python` 存在），
  v0.8.0 是在 CI 红的状态下发布的。教训进方案：发布 DONE 须含「CI 结论 = success」。

### 数字

unit 2484 · concurrency 33 · timeout 4 · e2e 25 · lab 26 · 闸门 I 8/8 · CI = success。

## [0.8.0] — 2026-09-12

**v0.7 把数据留下了；v0.8 把钱和权限的硬伤补上、把三轮验收与外部 review 挖出来的存量清完、
把顺延两次的 runtime contract + Python SDK 做出来。**

本版由三个 alpha 打磨而成：alpha.1（闸门 G 六条）· alpha.2（六条并行 lane + contract/SDK）·
alpha.3（R5 四课题复跑 + 两批修复窗口），最后经 A7 第六次零上下文验收再修一轮。
**每条门禁都有实跑的阴性对照；每份子代理报告都由主会话独立复现后才采信**——这一版里
R5 与 A7 各有一条「自报缺陷」经复核不成立（V124 / V129），也各有一条我起初判断错、它是对的（V132）。

## 给用户的亮点

- **钱：预算闸真的拦得住并发**。V93 封住单进程 `Promise.all`；V125 进一步把在飞预留按项目聚合到
  进程级注册表——A7 实测的「10 并发 × `budgetUsd=0.03` 实花 $0.0807」不再发生。
  **V94**：单价表查不到的模型**默认拒绝**，`--allow-unpriced` 显式放行并在 usage 标记。
- **权限：HTTP 审批不再是旁路**（V95）。`lab approve/simulate` 需要 `actor` + 一次性令牌，
  令牌由 `spark-research lab token <id>`（真实 TTY 门）签发、10 分钟、**单次消费**，
  无令牌一律 403 并指引去终端取。**不接任何真实设备**。
- **对外集成面**：`spark-research contract --json` 把 CLI/HTTP/MCP/配置/导出 manifest schema
  全部从真源派生成一份可版本化契约（78 路由 · 30 MCP 工具 · 12 条对撞门禁），随 Release 挂
  `contract.json`；**Python SDK** 由契约生成（`sdk/python`，78 方法 + 56 条 pytest 往返）。
- **安全**：`workspaceRoot` 不再接受项目外路径（V100）· 扩展 `--trust` 指纹改目录清单、改 helper
  必须重新授信（V101）· `auth` 录 key 不回显（V115）· CI 旁路 token 常量时间比较且可走环境变量（V116）。
- **数据完整性**：`state.json` 原子写（V96）· 仿真执行进 raw（V85）· **raw 链对 >64KB 单行不再静默断开**
  （V121，R5 四处断链的真因）· HTTP/SDK 的 `body.project` 不再被忽略（V122，此前会写错项目）。
- **检索与核验**：有礼貌池下 T1 recall@10 **5/8**（R4 当时全线 0/8）· `lit review` 打出
  「解析 N / 判定 M / 差额：去重 x · 自引 y · 解析失败 z」· 中文引用 key 对两道核验门可见（V92）。
- **体验**：精读进度每篇回传 · 项目下拉显示 slug · 引用跳证据图 · 精读/综述/novelty/聊天都有预算入口。

## 如实交代（未达成与残余）

- **DONE 定义未全部满足**，不粉饰：
  - 「BACKLOG 真待办 ≤ 5 条」**未达成**。开放条目约 20 条，绝大多数已标明去向（等外部硬件/凭据、
    或明确推 v0.9）；本版新登记的 V130/V131/V133 也在其中。
  - 「四课题召回每个 ≥ 基线 +2」**部分达成**：T1 5/8（基线 3/8，+2 ✓）；T2 2/8、T4 DOI 口径 1/8
    （标题匹配后 4/8）、T3 中文 0/3 —— 机制解释见 R5 报告（上游限流与 AMiner 拆词查准率），
    不是排序回归。
- **跨进程在飞预留仍不共享**（V125 残余）：两个 CLI 进程同时起跑时，越界上界是「每进程一次调用的估价」；
  同进程内（含 HTTP server）已封死。
- **raw 历史断点不回填**：v0.8 之前写坏的链仍然是断的，`data import` 会如实报出行号——回填等于伪造。
- **A7 有三项「跑不了」**：浏览器 UI 视觉交互（验收环境无浏览器工具，全部 curl 代跑）、
  令牌单次消费链条（`lab token` 强制真实 TTY，验收环境非 TTY）、以及一次因契约漏字段导致的误判（V132，已修）。
- **明确不做**：物理设备路径 · 售卖通道/计费/许可文本 · 3D 查看器 · 多用户身份 · R kernel。

## 数字

unit **2445**（v0.7.0 时 2271）· concurrency+timeout **37** · e2e **25** · py **131**（含 SDK 56）·
lab 26 · 二进制冒烟 exit 0 · 契约 78 路由 / 30 MCP 工具 / 32 配置项。

## 安装

```bash
# 单二进制（零预装）
gh release download v0.8.0 --pattern 'spark-research-*'
# 源码
git clone https://github.com/jimmyag2026-prog/spark-research && cd spark-research && bun install
# Python SDK
pip install -e sdk/python
```

## [0.8.0-alpha.3] — 2026-09-12

**R5 四课题零上下文复跑 + 修复窗口。** R5 报告的每条 P0/P1 都由主会话**独立复现后才动手**——一条证伪、两条证实并挖出真因。

### R5 实证回环（四课题，v0.8.0-alpha.2 基线）

- **召回回升**：T1 recall@10 **5/8**（R4 全线 0/8——当时 OpenAlex 匿名池 100% 429）；T2 2/8，T4 标题匹配后 4/8，T3 中文连写复合词 jieba 兜底生效。
- **R4 的两条 P0 已确认修复**：`data export --for-sharing` 不再泄漏上游摘要（4/4 项目 grep 不命中）；connector/kernel raw 不再落全局目录。
- **预算闸并发下严格生效**：10 个并发请求在真正调用 LLM 前全部被拒，`knownCostUsd` 前后不变。
- 判定器 JSON 失败率四课题全 0%（含第二模型 kimi-k2.6）；并发 `--project` 隔离 130 条 record 零串项目；kill -9 → orphaned → 断点续跑不重复花钱。

### 修复（第一批）

- **V104** HTTP 综述与 CLI 同一语义：落 citation-integrity observation record，响应带 `citationReviewRecordId`/`citationGap`。
- **V115** `auth` 录 key 不回显（TTY raw 逐字读，管道按行）。
- **V116** CI 旁路 token：sha256 + `timingSafeEqual` 常量时间比较；新增 `--ci-bypass-token-env <VAR>`，token 不必走 argv。
- **V119** 聊天式 chat/co-explore 接上预算闸（UI 预算入口 → 路由透传 → 按会话记预算）；闸拒绝时把闸消息原样回给用户，不再误报"检查 API key"。
- **V118** openmm 探测与真提交同源（runner 暴露 `probe()`），四平台口径一致，P5 契约测试不再整套 skip。

### 修复（第二批 · R5 复核）

- **V121 raw 链在单行 >64KB 时静默断开**：`readTail` 固定 64KB 窗口，窗口整个落在该行内部时把**残行**当完整行返回 → `JSON.parse` 抛错被吞成 `last=null` → 下一条写出 `prevHash:null`。真因实证：R5 四处断链的前一行分别是 68444/68775/70330/68398 字节，**全部 >64KB 无一例外**；llm 链免疫（`body()` 把大正文移进 blobs）。修：窗口按需放大（64KB→1MB→16MB→整文件）+ 残行不解析 + 读不懂直接抛。**历史断点不回填**（回填等于伪造）。
- **V122 HTTP/SDK 的 `body.project` 被静默忽略**：写请求落进全局当前项目指针，响应还回报错误的 project 名——而 SDK 生成的正是 body 风格。修：取值顺序 **query > body > 当前指针**，一处覆盖全部 47 个写路由。
- **V123 SDK 无 body 的 POST 一律 415**：写方法在 body 为空时按 `{}` 发送并带 JSON 头；`docs/SDK.md` 删去与实测不符的返回类型断言。

### 如实交代

- **V124：R5 报的「导出往返 report diff 非空」复核不成立**——两侧都带 `--verbose` 时 diff 为 0 行，只有旗标不对称才复现它描述的形状。方法学偏差，不是产品缺陷，已登记防止再次登记。
- V121 的历史断点不回填：R5 那几个项目的 connector 链仍是断的，`data import` 会如实报出行号。
- R5 验证过程中因 V122 把 3 篇论文误写进无关项目，已 tombstone 撤回。

### 数字（alpha.2 → alpha.3）

unit 2420 → **2438** · concurrency+timeout 37 · e2e 25 · py 129 → **131** · lab 26 · test:sdk 58 · 冒烟 exit 0。
