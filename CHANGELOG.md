...
- **一轮 chat 没有变快。** R6 基线：一句话问题 P50 27–101s、每轮 3–5 次模型调用；机制已量清楚（`docs/devlog/R6-baseline.md` §机制解释：109.6s 里 78% 是模型在生成 8590 个输出 token；chat 对任何问题都走完整 plan→execute→summarize，各阶段无输出上限）。DONE 第 2 条走的是「机制解释」分支。压它是 v0.10 的第一件事（直答路径 + 各阶段 `maxTokens`）。
