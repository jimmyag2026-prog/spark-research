...
5. **审批类动作你调不到，这是设计不是缺陷。**
   ...
   CLI 层要求一次交互终端里的显式 `yes`——湿实验与计费动作必须人工批准（AD-9/AD-14）。
   **如实说明（V167 实测）**：那道 TTY 检测**不是安全边界**——`script -q /dev/null <命令>`
   这种 pty 包装器就能满足它，把 `yes` 喂进去就能批准。它挡的是误触与「顺手回车」，
   以及让非交互旁路必须显式带 token + reason 从而留痕；它挡不住一个有意为之的自动化。
   审批真正成立靠的是 decision record 的 actor/hash（事后可追责）与 MCP_WITHHELD
   （子代理的默认路径上拿不到这两个动作）。
