...
【想避免超时】**两个上限要一起调**，只调一个没用：
  ...
  · SPARK_RESEARCH_TASK_TIMEOUT_MS=900000 —— 任务本身的生命周期上限，**默认只有 600000（10 分钟）**，超过它任务会被结构化地判为超时失败（旧名 SPARK_TASK_TIMEOUT_MS 已于 v0.8 移除，设了会直接报错并提示改名）
...
【所以更该做的是别让它超时】调大**两个**上限（只调一个没用）：SPARK_RESEARCH_MCP_TIMEOUT_MS（MCP 等待）与 SPARK_RESEARCH_TASK_TIMEOUT_MS（任务生命周期，默认 600000 即 10 分钟；旧名 SPARK_TASK_TIMEOUT_MS 已于 v0.8 移除，设了会直接报错）；或缩小单次范围（如 lit_read_cards 用 tag 分批）。
