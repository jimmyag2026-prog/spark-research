...
`volume_capacity` 全程可信；`chemical_compatibility` 认识一个有限的中英文/分子式试剂词表
（词表外的试剂完全看不见）。

`concentration_limit` / `biosafety` 从 V25 起**不再恒空转，但覆盖是部分的**：
- `concentration_limit` 只在**同一子句里恰好点名一种试剂**时才吃得到浓度（例如
  「配制10%次氯酸钠溶液」）。同一子句出现两种及以上试剂、或浓度描述和试剂名分处
  不同分句（例如「配制次氯酸钠，浓度为10%」），编译器不瞎猜归属，这条规则仍然
  拿不到输入。
- `biosafety` 能挂到「这句话最终归属的步骤」（本句新建的步骤，或它作为续句合并进的
  上一步）。一句独立的生物安全描述、前面没有任何步骤可挂时，同样拿不到输入。

**跨句归属（先说试剂、后单独一句说浓度/生物安全等级）这种最常见的写法，两条规则
目前都不认**——那种情况下 `unconsumedWarnings` 仍然会报，`lab status` / HTTP 响应体
都能看到，不会静默放行。写协议时想让这两条规则真的生效，把浓度/生物安全等级和
对应的试剂/步骤写在**同一个分句**里。
